...
class BhavcopyService:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_bytes) as z:
                for file_name in z.namelist():
                    ...
                    self._info(f"📄 Processing {base} from manual URL")
    ...
    def process_zip_for_date(self, date_obj: datetime, force_refresh: bool = False):
        ...
        try:
            ...
            if resp.status_code == 404:
                ...
                return self._with_duration(started_at, {"date": str(date_key), "status": "NOT_FOUND", "message": "No Bhavcopy available for this date"})
            
            if resp.status_code != 200:
                logger.error(f"❌ Failed to fetch {date_key}: HTTP {resp.status_code}")
                return self._with_duration(started_at, {"date": str(date_key), "status": "FAILED", "message": f"HTTP {resp.status_code}"})
            ...
            with zipfile.ZipFile(zip_bytes) as z:
                ...
                for file_name in z.namelist():
                    ...
                    if not force_refresh and self.is_date_processed(base, date_obj):
                        self._info(f"⏭ {base} data for {date_key} already exists, skipping")
                        result_data[base] = {"status": "ALREADY_EXISTS"}
                        continue

                    self._info(f"📄 Processing {base} for {date_key}")
    ...
    def fetch_today_bhavcopy(self, force_refresh: bool = False):
        ...
        if today.weekday() >= 5:
            self._info(f"📅 {today.date()} is weekend, skipping Bhavcopy fetch")
            return self._with_duration(time.perf_counter(), {"date": str(today.date()), "status": "WEEKEND", "message": "No Bhavcopy available on weekends"})
