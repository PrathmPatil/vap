...
export const FORMULA_EXPLORE_ITEMS: FormulaExploreItem[] = [
  ...
  },
  {
    ...
    shortDescription: "Strong green close versus the open.",
    fullDescription:
      "Screens for stocks with meaningful intraday buying pressure — close up versus open by at least the threshold %. Does not require a dominant real body.",
    rules: [
      "Close change ≥ threshold % (default 2%)",
      "Uses OPEN and CLOSE from bhavcopy",
      "EQ-only universe",
    ],
    tags: ["Candle", "Bullish"],
    premium: true,
  ...
  {
    value: "strong-king-candle",
    label: "Strong King Candle",
    categoryId: "candle-patterns",
    shortDescription: "Strong close gain with a dominant real body.",
    fullDescription:
      "Finds king candles where the close rises meaningfully versus the open and the real body covers most of the day's high–low range. Stricter than Strong Bullish Candle.",
    rules: [
      "Close change ≥ threshold % (default 2%)",
      "Body ≥ configurable % of high–low range (default 80%)",
      "Uses OPEN, HIGH, LOW, CLOSE from bhavcopy",
    ],
    tags: ["Candle", "Bullish", "King"],
    premium: true,
  },
  {
